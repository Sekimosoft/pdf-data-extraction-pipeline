...
async function captureInvoice(page, locale, suffix) {
  ...
  await page.waitForTimeout(800);
  await page.screenshot({ path: `docs/screenshots/screenshot-${suffix}-structured.png`, fullPage: true });
}
...
async function captureTextPreview(page) {
  ...
  await page.waitForTimeout(800);
  await page.screenshot({ path: "docs/screenshots/screenshot-en-text-preview.png", fullPage: true });
}
